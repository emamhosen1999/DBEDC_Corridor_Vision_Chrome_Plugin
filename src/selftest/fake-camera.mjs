/**
 * A simulated IP camera: RTSP on one port, HTTP (ONVIF + snapshot) on another.
 *
 * Enough of each protocol to exercise the probe ladder for real — Digest auth, SDP,
 * and a JPEG snapshot — so `selftest` can prove the pipeline works on a machine that
 * has no cameras on it yet, or cannot reach them.
 *
 * `behaviour` selects the fault to simulate:
 *   healthy    — serves everything correctly
 *   rtsp-dead  — answers ONVIF but refuses to serve video (the class a VMS calls
 *                "online", and the reason this product exists)
 *   silent     — accepts the connection and never replies
 */
